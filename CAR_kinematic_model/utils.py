...
def make_square(x, y, width):
    outline = np.array([[x - int(width / 2), i] for i in range(y - int(width / 2), y + int(width / 2))] +
                      [[x + int(width / 2), i] for i in range(y - int(width / 2), y + int(width / 2))] +
                      [[i, y - int(width / 2)] for i in range(x - int(width / 2), x + int(width / 2))] +
                      [[i, y + int(width / 2)] for i in range(x - int(width / 2), x + int(width / 2))])

    interior = np.array([[i, j] for i in range(x - int(width / 2) + 1, x + int(width / 2)) for j in
                         range(y - int(width / 2) + 1, y + int(width / 2))])

    square = np.concatenate((outline, interior))
